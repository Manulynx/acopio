...
class ProductoForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            'codigo': forms.TextInput(attrs={
                'class': 'form-control',
                'placeholder': 'Código del producto (opcional - se genera automático)'
            }),
    ...
    def clean(self):
        ...
        if requiere_vencimiento and not dias_vida_util:
            raise ValidationError({
                'dias_vida_util': 'Debe especificar los días de vida útil si el producto requiere control de vencimiento.'
            })
...
class LoteProductoForm(forms.ModelForm):
    class Meta:
        model = LoteProducto
        fields = [
            'producto', 'numero_lote', 'fecha_produccion', 'fecha_vencimiento',
    ...
    def clean(self):
        ...
        # Validar que el producto requiera control de lotes
        if producto and not producto.requiere_lote:
            raise ValidationError({
                'producto': 'Este producto no requiere control por lotes.'
            })

        # Validar fecha de vencimiento si el producto lo requiere
        if producto and producto.requiere_vencimiento and not fecha_vencimiento:
            raise ValidationError({
                'fecha_vencimiento': 'Este producto requiere fecha de vencimiento.'
            })

        # Validar unicidad del lote para el producto
        if producto and numero_lote:
            existing_lote = LoteProducto.objects.filter(
                producto=producto, 
            ...
            if existing_lote.exists():
                raise ValidationError({
                    'numero_lote': f'Ya existe un lote con este número para el producto {producto.nombre}.'
                })
...
class MovimientoStockForm(forms.ModelForm):
    class Meta:
        ...
        fields = [
            'producto', 'lote', 'tipo_movimiento', 'cantidad',
    ...
    def clean(self):
        ...
        if producto and lote and lote.producto != producto:
            raise ValidationError({
                'lote': 'El lote seleccionado no pertenece al producto elegido.'
            })
            ...
            if producto and cantidad:
                if lote:
                    ...
                    if cantidad > producto.stock_actual:
                        raise ValidationError({
                            'cantidad': f'No hay suficiente stock del producto. Disponible: {producto.stock_actual}'
                        })
